...
Public Class frmReadyBlitz
    ...
    Private Sub INICIALIZAR_FORM()
        ...
        rdrMex50 = dsOpenDB(comm)
        grdMex50.DataSource = rdrMex50.Tables(0)
        grdMex50.DataBind()


        'MÉXICO 100
        sSql = "SELECT ESTADO_MUNICIPIO, TIPO_BLITZ, COUNT(*)  AS TOTAL"
        sSql &= "FROM ARCHIVOS_DATA LEFT JOIN TAB_CP ON TAB_CP.CP = ARCHIVOS_DATA.CP INNER JOIN TAB_ESTADOS ON TAB_ESTADOS.ID_ESTADO = TAB_CP.ID_ESTADO "
        ...
        rdrMex100 = dsOpenDB(comm)
        grdMex100.DataSource = rdrMex100.Tables(0)
        grdMex100.DataBind()

        'MÉXICO 300
        sSql = "SELECT ESTADO_MUNICIPIO, TIPO_BLITZ, COUNT(*)  AS TOTAL"
        sSql &= "FROM ARCHIVOS_DATA "
        sSql &= "LEFT JOIN TAB_CP ON TAB_CP.CP = ARCHIVOS_DATA.CP "
        sSql &= "INNER JOIN TAB_ESTADOS ON TAB_ESTADOS.ID_ESTADO = TAB_CP.ID_ESTADO "
        ...
        'ESTADOS 50
        sSql = "SELECT DESC_ESTADO, TIPO_BLITZ, COUNT(*)  AS TOTAL"
        sSql &= "FROM ARCHIVOS_DATA LEFT JOIN TAB_CP ON TAB_CP.CP = ARCHIVOS_DATA.CP "
        sSql &= "INNER JOIN TAB_ESTADOS ON TAB_ESTADOS.ID_ESTADO = TAB_CP.ID_ESTADO "
        sSql &= "WHERE TIPO_BLITZ = 'BLITZ50' "
